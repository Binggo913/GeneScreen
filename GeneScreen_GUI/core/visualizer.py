...
class LinkviewVisualizer:
    ...
    def __init__(self, output_dir: str, min_aln_len: int = 100,
        ...
        Args:
            output_dir: 输出目录
            min_aln_len: 最小比对长度阈值，过滤 (LEN1+LEN2)/2 <= 此值的比对块 (默认 100bp)
...
class SequenceVisualizer(BaseVisualizer):
    ...
    def visualize(
        ...
        if target_suffix is None and len(result.get("query_results") or []) > 1:
            target_suffix = self._legacy_report_suffix_for_result(result, genome_files)
        report_file = self._legacy_report_path(safe_seq_id, target_suffix)
        with open(report_file, "w", encoding="utf-8") as f:
            f.write(html)
...
class ReportGenerator:
    ...
    def generate_report(self, result, mode, ref_name="", qry_name=""):
        ...
        Returns:
            报告文件路径
        """
        identity = result.get("identity", 90)
        min_aln_len = result.get("min_aln_len", 100)
        input_source = result.get("input_source")
        genome_files = result.get("genome_files")
        
        # 获取查询基因组的 .fai 索引文件路径（用于读取染色体长度）
        ref_index_file = None
        if genome_files:
            qry_fasta = genome_files.get("qry_fasta")
            if qry_fasta:
                fai_path = f"{qry_fasta}.fai"
                if os.path.exists(fai_path):
                    ref_index_file = fai_path
        
        if mode == "gene_id":
            viz = GeneIDVisualizer(
                self.output_dir,
                min_aln_len=min_aln_len,
                ref_index_file=ref_index_file,
                min_identity=identity,
            )
            return viz.visualize(result, ref_name, qry_name, identity, input_source, genome_files)
        elif mode == "location":
            ...
            # Sequence 模式：查询的是参考基因组，所以用 ref_fasta 的 .fai
            if genome_files:
                ref_fasta = genome_files.get("ref_fasta")
                if ref_fasta:
            ...
            return viz.visualize(result, ref_name, None, identity, input_source, genome_files)
